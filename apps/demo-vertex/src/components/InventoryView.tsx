    ...
    <section className="w-full max-w-6xl mx-auto" aria-labelledby="inventory-heading">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div><h1 id="inventory-heading" className="text-2xl sm:text-3xl font-bold">إدارة المخزون <span className="text-gray-500 font-normal text-xl">- الجرد</span></h1><p className="text-sm text-gray-600">إدارة مستويات المخزون، الأسعار وتفاصيل المنتجات</p></div>
        </div>
            ...
            <tbody>{inventory.map((row) => {
              ...
              return (
                <tr key={row.itemId} className="border-b border-gray-100 last:border-0" data-testid={`inventory-${row.code}`}>
                  <td className="py-3 px-3"><b>{row.name}</b><small className="block text-gray-500 font-mono" dir="ltr">{row.code}</small></td>
                  ...
                  <td className={`py-3 px-3 text-center font-bold ${low ? 'text-red-600' : 'text-text-light'}`}><span data-testid={`quantity-${row.code}`}>{formatQuantity(row.quantity)}</span> <small className="text-gray-500">{row.unit}</small></td>
